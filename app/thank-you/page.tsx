...
const SERVICES = {
  ...
  gallbladder: {
    message:
      "Our team will get back to you shortly to confirm your appointment for gallbladder evaluation and treatment.",
    backHref: "/gallbladder-surgery",
  },
  maternity: {
    message:
...
export default async function ThankYouPage({ searchParams }: PageProps<"/thank-you">) {
  ...
  const content =
    typeof service === "string" && service in SERVICES
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-[#EAF6F8] px-5 py-16 text-center">
      ...
      <p className="mt-3 max-w-md text-lg leading-relaxed text-[#3F5A66]">{content.message}</p>
